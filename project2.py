...
def apply_midpoint_filter():
    ...
    for y in range(img_array.shape[0]):
        for x in range(img_array.shape[1]):
            region = padded[y:y+3, x:x+3].astype(np.int16)   # <— cast here
            # compute midpoint of true max/min
            m = (int(region.max()) + int(region.min())) // 2
            filtered[y, x] = m
...
def show_histogram2():
    ...
    if filtered_img is None:
        update_status("No processed image to show histogram for.")
        return

    # Work on the filtered image (note: this is already resized to IMAGE_DISPLAY_SIZE)
    img_hist = filtered_img.convert("RGB")
    img_array = np.array(img_hist)

    plt.figure("Filtered Image Histogram")
    colors = ('r', 'g', 'b')
    for i, color in enumerate(colors):
        channel = img_array[:, :, i]
        plt.hist(channel.ravel(), bins=256, range=(0, 256), color=color,
                 alpha=0.5, label=f'{color.upper()} channel')
    plt.title("RGB Histogram (Filtered)")
    plt.xlabel("Pixel Intensity")
    plt.ylabel("Frequency")
    plt.legend()
    plt.grid(True)
...
btn_open.grid(row=7, column=2, padx=10)

btn_red = Button(frame_controls, text="Red Filter", command=red_filter)
btn_red.grid(row=0, column=0, padx=10)

btn_green = Button(frame_controls, text="Green Filter", command=green_filter)
btn_green.grid(row=0, column=1, padx=10)

btn_blue = Button(frame_controls, text="Blue Filter", command=blue_filter)
btn_blue.grid(row=0, column=2, padx=10)

Label(frame_controls, text="R:").grid(row=0, column=3)
entry_r = Entry(frame_controls, width=4)
entry_r.grid(row=0, column=4)

Label(frame_controls, text="G:").grid(row=0, column=5)
entry_g = Entry(frame_controls, width=4)
entry_g.grid(row=0, column=6)

Label(frame_controls, text="B:").grid(row=0, column=7)
entry_b = Entry(frame_controls, width=4)
entry_b.grid(row=0, column=8)

def apply_custom_rgb_filter():
    if not img:
        update_status("No image loaded.")
        return
    ...
    try:
        ...
        g_val = int(entry_g.get())
        b_val = int(entry_b.get())
        if not all(0 <= v <= 255 for v in [r_val, g_val, b_val]):
            raise ValueError
    except ValueError:
        update_status("RGB values must be integers between 0 and 255.")
        return

    img_rgb = img.convert("RGB")
    img_array = np.array(img_rgb)
    r_channel, g_channel, b_channel = img_array[:,:,0], img_array[:,:,1], img_array[:,:,2]
    r_mask = (r_channel * (r_val / 255)).astype(np.uint8)
    g_mask = (g_channel * (g_val / 255)).astype(np.uint8)
    b_mask = (b_channel * (b_val / 255)).astype(np.uint8)
    result = np.stack([r_mask, g_mask, b_mask], axis=2)
    result_img = Image.fromarray(result)
    show_image(result_img, panel_filtered)
    update_status("Custom RGB filter applied.")

btn_custom_rgb = Button(frame_controls, text="Apply RGB", command=apply_custom_rgb_filter)
btn_custom_rgb.grid(row=0, column=9, padx=10)

btn_gray = Button(frame_controls, text="Gray Filter", command=gray_filter)
btn_gray.grid(row=1, column=3, padx=10, pady=10)
...
btn_histogram = Button(frame_controls, text="original Histogram", command=show_histogram)
btn_histogram.grid(row=7, column=0, padx=5, pady=5)

btn_histogram_filtered = Button(frame_controls,
    text="Filtered Histogram",
    command=show_histogram2)
btn_histogram_filtered.grid(row=7, column=1, padx=5, pady=5)


btn_save = Button(frame_controls, text="Save Output", command=save_output)
btn_save.grid(row=7, column=3, padx=10, pady=10)
